...
function getFormValues () {
    ...
    const desiginEl = document.querySelector ('#desigin');
...
const forEl = document.querySelector('#project-price-form');
const emailModal = document.querySelector('#modal-email');
const successModal = document.querySelector('#success-modal');

//Первий просчет форми
calculateWork();

forEl.addEventListener('change' , calculateWork);

forEl.addEventListener('submit', function(event) {
...
closeButtons.forEach( function(closeBtn) {
    closeBtn.addEventListener('click', function () {
        const inputContainer = document.querySelectorAll('#email-input-container');
...
modalEmailContainer.addEventListener('submit', function(event) {
    ...
    if(userEmailInput.value) {
        ...
        const formData = new FormData(forEl);
